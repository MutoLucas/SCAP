        ...
        <div class="d-flex justify-content-between p-2 text-success">
            ...
            <div class="d-flex justify-content-between gap-2">
                <div class="input-group">
                    <input wire:model.lazy="filterId" type="text" class="form-control form-control-sm"
                        placeholder="Ex: 2142578">
                </div>
                    ...
                    @foreach ($this->paradas as $parada)
                                ...
                                @if (auth()->check())
                                    <div class="d-flex justify-content-center gap-2">
                                        <button class="btn btn-sm btn-primary"
                                            wire:click="editLine({{ $parada->Id }})"><i
                                        ...
                                        @if (auth()->user()->NivelAcesso == 'Administrador')
                                            <button class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                                ...
                                                wire:click="selectToDelete({{ $parada->Id }})"><i
                                                    class="bi bi-x-square"></i></button>
                                        @endif

                                        <button class="btn btn-sm btn-success"
                                            wire:click="selectToDivide({{ $parada->Id }})" data-bs-toggle="modal"
                                            data-bs-target="#divideLine">
                                            <i class="bi bi-hr"></i>
                                        </button>
                                    </div>
                                @else
                        ...
                        <div class="modal fade" id="divideLine" data-bs-backdrop="static" wire:ignore.self>
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title fw-bold">Dividir Linha:
                                            {{ $selectedToDivide->Id ?? '' }}</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                    </div>

                                    <div class="modal-body">
                                        @if (isset($messageDivideLine[$parada->Id]['message']))
                                            <div class="alert alert-{{ $messageDivideLine[$parada->Id]['severity'] }} alert-dismissible fade show"
                                                role="alert">
                                                <strong>
                                        ...
                                        @if ($selectedToDivide)
                                            <div class="w-full mb-3">
                                                ...
                                                <input type="datetime-local"
                                                    value="{{ \Carbon\Carbon::parse($selectedToDivide->DataInicio)->format('Y-m-d\TH:i') }}"
                                                    class="form-control" disabled>
                                            </div>

                                            <div class="w-full mb-3">
                                                <label class="form-label">Data Fim</label>
                                                @if ($selectedToDivide->DataFim)
                                                    <input type="datetime-local"
                                                        value="{{ \Carbon\Carbon::parse($selectedToDivide->DataFim)->format('Y-m-d\TH:i') }}"
                                                        class="form-control" disabled>
                                                @else
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary"
                                            data-bs-dismiss="modal">Fechar</button>
                                        @if ($selectedToDivide && $selectedToDivide->DataFim)
                                            <button class="btn btn-success" type="button"
                                                wire:click="divideLine({{ $parada->Id }})"><i class="bi bi-hr"></i>
                                                Dividir</button>
                                        @endif
                                    </div>
                                </div>
