            ...
            <div class="mb-2">
                ...
                <select class="form-select form-select-sm border-primary" wire:model.lazy="processo">
                    ...
                    @foreach ($this->processos as $processo)
                        <option value="{{ $processo->Processo }}">{{ $processo->Processo }} - {{ $processo->Descrição }}
                        </option>
                    @endforeach
                </select>
                @error('processo')
                ...
                <div class="input-group">
                    <select class="form-select form-select-sm border-primary" wire:model="operador">
                        ...
                        @foreach ($this->operadores as $operador)
                            <option value="{{ $operador->Login }}">{{ $operador->Nome }} - {{ $operador->Login }}
                            </option>
                        @endforeach
                    </select>
                    <input type="text" class="form-control form-control-sm border-primary" placeholder="Buscar.."
                        wire:model.lazy="searchOperador">
                </div>
                @error('operador')
                    <small class="text-danger fw-bold">{{ $message }}</small>
                @enderror
